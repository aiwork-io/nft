	...
	function password_hash($password, $algo, array $options = array())
	{
		...
		elseif ( ! isset($options['salt']))
		{
			if (function_exists('random_bytes'))
			{
				...
				{
					\Yii::error('compat/password: Error while trying to use random_bytes(): '.$e->getMessage());
					return FALSE;
				}
			}
			...
			elseif (DIRECTORY_SEPARATOR === '/' && (is_readable($dev = '/dev/arandom') OR is_readable($dev = '/dev/urandom')))
			{
				...
				{
					\Yii::error( 'compat/password: Unable to open '.$dev.' for reading.');
					return FALSE;
				}
				...
				for ($read = 0; $read < 16; $read = ($func_overload) ? mb_strlen($options['salt'], '8bit') : strlen($options['salt']))
				{
					...
					{
						\Yii::error( 'compat/password: Error while reading from '.$dev.'.');
						return FALSE;
					}
					$options['salt'] .= $read;
				}
			...
			elseif (function_exists('openssl_random_pseudo_bytes'))
			{
				...
				if ($is_secure !== TRUE)
				{
                    \Yii::error( 'compat/password: openssl_random_pseudo_bytes() set the $cryto_strong flag to FALSE');
					return FALSE;
				}
			}
			else
			{
                \Yii::error('compat/password: No CSPRNG available.');
				return FALSE;
			}
